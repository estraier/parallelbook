...
- --assistant : アシスタントAPIを使ってタスク間で記憶を継承します。
...
デフォルトでは、ChatGPTのcompletion APIが使われます。これはステートレスAPIであり、各タスクの記憶が共有されません。よって、文脈を加味した翻訳を行うには、各タスクで前後の文脈情報を明示的に与えることが必要です。その反面、各タスクを個別に実行しても常に同じ結果が得られるという利点があります。一方で、--assistantオプションをつけると、assistant APIが使われます。これはステートフルAPIであり、各タスクで記憶が共有されるようになります。この場合、前の文脈は与えなくても、前のタスクの記憶が文脈情報として用いられます。より過去分に関してはより広い文脈情報を拾える利点の反面、それまでのタスクによって結果が変わるという不安定さが導入される欠点があります。

completion APIとassistant APIのどちらが良いのかは、一概には言えません。双方で使うプロンプトは同じなので、コストは同じで、違いは記憶がタスク間で継承されるかどうかだけです。タスク間で記憶が共有されるので、長期間の文脈が維持されるとともに、訳語の語彙に一貫性が増すことも期待されます。一方で、誤認識や誤訳も継承されるので、それによって広範囲の結果が汚染される危険性もあります。

実際のところ、completion APIでも前文脈500文字および後文脈200文字を与えているので、個々のタスクで文脈情報が不足する確率はとても低いです。assistant APIによる翻訳精度の向上があるとすれば、ChatGPTが自ら生成した訳文を覚えていることに起因します。語彙や語調の統一感がその典型です。多くの場合、completion APIで十分かつ安定した精度が出ますが、語彙や語調の不統一が気になる場合にはassistant APIに切り替えると良いでしょう。
